...
def decompose_gaussian(data: np.ndarray, sigma: float = 10) -> tuple:
    """
...
def decompose_bilateral(
    ...
    Smooths while preserving edges. Good for discontinuities.
    """
    data_filled = np.nan_to_num(data, nan=np.nanmean(data))
    ...
    data_norm = ((data_filled - data_min) / data_range * 255).astype(np.float32)
    
    # Apply bilateral filter
    trend_norm = cv2.bilateralFilter(data_norm, d, sigma_color, sigma_space)
    
    # Rescale back to original range
    trend = trend_norm / 255 * data_range + data_min
    residual = data_filled - trend
...
def decompose_tophat(
    ...
    White top-hat: extracts bright features smaller than element
    Black top-hat: extracts dark features smaller than element
    """
    ...
    if mode == 'white':
        residual = white_tophat(data_filled, footprint=selem)
    else:
        residual = black_tophat(data_filled, footprint=selem)
...
@register_decomposition(
    ...
    destroys='large-scale slope/curvature'
)
def decompose_polynomial(data: np.ndarray, degree: int = 2) -> tuple:
    """
